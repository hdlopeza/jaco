...
ds_val = tf.data.TFRecordDataset('/project/data/invoices_val.tfrecord')
ds_train = tf.data.TFRecordDataset('/project/data/invoices_train.tfrecord')
ds_test = tf.data.TFRecordDataset('/project/data/invoices_test.tfrecord')
...
inputs = tf.keras.Input(shape=(utils.alto*utils.ancho*3,), name='images')
...
ds_train = ds_train.shuffle(buffer_size=32).batch(8)
ds_test = ds_test.batch(6)  
