...
#! HYPERPARAMETROS
ERODE_ITERATIONS = 0 #1
DILATE_ITERATIONS = 3 #6
HIGHT_MAX = 40 # 40
HIGHT_MIN = 9 # 16
THRESHOLD_COLUMN = 12 #29
THRESHOLD_ROW = 2 #29

def mostrar(titulo, image):
...
for box in boxes:
    (x, y, w, h) = box
    cv2.rectangle(_img, (x, y), (x + w - 2, y + h - 2), (0, 255, 0), 1)
mostrar('5', _img)
#----------
...
for box in boxes:
    (x, y, w, h) = box
    cv2.rectangle(_img, (x, y), (x + w, y + h), (0, 0, 255), 1)
...
#                               imgx[punto_fila[i]-3:punto_fila[i+1], 
...
# result = []
# for box in boxes:
#     result.append(pytesseract.image_to_string(imgx[
#         box[1]-1:box[1]+box[3], box[0]:box[0]+box[2]]))
# result
