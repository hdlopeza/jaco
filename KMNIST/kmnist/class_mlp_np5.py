...
images_flatten_test = np.array([_.reshape((28*28,)) for _ in images_test])

ds_train = tf.data.Dataset.from_tensor_slices((images_flatten_train, labels_train)).batch(32).repeat()
ds_test = tf.data.Dataset.from_tensor_slices((images_flatten_test, labels_test)).batch(32).repeat()
...
model.fit(
    ...
    validation_steps=20) # el 10% de steps per epoch